gollum = {
	...
	move = function(mob, target)
		...
		if
			(distance(mob, target) <= 10 and mob.registry["gollum_stone_connection_aether"] < os.time() and target.state ~= 1 and
				mob.m == target.m)
		 then
	...
	throw_stone = function(mob, target)
		...
		if (#pcBlocks > 0) then
			for i = 1, #pcBlocks do
				if
					((pcBlocks[i].x == mob.x and (pcBlocks[i].y == mob.y - 1 or pcBlocks[i].y == mob.y + 1)) or
						(pcBlocks[i].y == mob.y and (pcBlocks[i].x == mob.x - 1 or pcBlocks[i].x == mob.x + 1)) or
						((pcBlocks[i].x == mob.x - 1 or pcBlocks[i].x == mob.x + 1) and
							(pcBlocks[i].y == mob.y - 1 or pcBlocks[i].y == mob.y + 1)) and
							pcBlocks[i].state ~= 1)
				 then
