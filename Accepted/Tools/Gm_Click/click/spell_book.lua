		...
		repeat
			for i = 1, #spell do
				if menu2 == i then
					...
					choice =
						player:menuSeq(name .. 'You are about to remove "' .. spell[i] .. '" spell from target.\nConfirm?', opts, {})
					if choice == 1 then
						if player.ID ~= 2 and player.ID ~= 166 then -- other GMs can't make change at your character. You are the God!
							if target.ID == 2 or target.ID == 166 then
								target:msg(
									4,
									"[INFO] " .. player.name .. " is trying to remove " .. spell[i] .. " from your spellbook!",
									target.ID
								)
								return
							end
