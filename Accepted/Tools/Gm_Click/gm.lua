			...
			if player.registry["gm_pin"] == 0 then
				...
				if first > 0 then
					if string.len(first) ~= 6 then
						player:dialogSeq(
							{t, name .. "ERROR!\n\nPIN must in 6 digits of number and do not use zero(0) as the first digit!"},
							1
						)
						player:freeAsync()
						gm.createPIN(player)
						...
						if confirm == first then
							menu =
								player:menuString(
								name .. "You're about to use '" .. confirm .. "' as your GM PIN?",
								{"Confirm PIN", "Let me retype", "Exit"}
							)
				...
				player:dialogSeq(
					{
						...
						name ..
							"This will proof that if you are the owner of this GM character! Of course if you failed or entered a wrong PIN, then your character will no longer have the GM Powers and access to it! (Command & GM Tools)",
						name ..
							"Because this is the first time you do login since we are decided to use this security system for any GMs, then you must to create a new 'GM PIN' for your character (" ..
								player.name .. ")"
					},
					1
				)
