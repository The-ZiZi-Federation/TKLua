petrify = {
	on_learn = function(player)
		player.registry["learned_petrify"] = 1
	end,
	on_forget = function(player)
		player.registry["learned_petrify"] = 0
	end,
	cast = function(player, target)
		...
		if (target.paralyzed == true) or target:hasDuration("petrify") then
		...
		if checkResist(player, target, "petrify") == 1 then
			return
		end
