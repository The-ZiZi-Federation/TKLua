...
cone_of_cold_lv2 = {
	on_learn = function(player)
		player.registry["learned_cone_of_cold_lv2"] = 1
	...
	on_forget = function(player)
		player.registry["learned_cone_of_cold_lv2"] = 0
	end,
			...
			if (mobTargets[i] ~= nil) then
				...
				if r <= 50 then
					if not mobTargets[i]:hasDuration("slow") then
						if checkResist(player, mobTargets[i], "slow") == 1 then
							return
						end
	...
	requirements = function(player)
		...
		local desc = {
			"Cone of Cold Lv2 will send a cone of ice in front of you, chilling your enemies!\n\nReplaces Cone of Cold Lv1",
			txt
		}
