...
cure_critical_wounds = {
	...
	on_forget = function(player)
		player.registry["learned_cure_critical_wounds"] = 0
	end,
	cast = function(player, target)
		...
		if magicCost > 420 then
			magicCost = 420
		end
		if healAmount > 6600 then
			healAmount = 6600
		end
				...
				if target ~= nil then
					if target.state == 1 or player.health <= 0 then
						...
						if target.blType == BL_PC and target.ID ~= player.ID then
							target:sendMinitext(
								player.name .. " cast Cure Critical Wounds on you.  You are healed for " .. healAmount .. " Vita."
							)
						end
					end
				end
