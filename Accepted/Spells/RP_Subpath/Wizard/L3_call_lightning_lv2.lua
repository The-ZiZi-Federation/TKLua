...
call_lightning_lv2 = {
	on_learn = function(player)
		player.registry["learned_call_lightning_lv2"] = 1
		player:removeSpell("call_lightning_lv1")
	end,
	on_forget = function(player)
		player.registry["learned_call_lightning_lv2"] = 0
	end,
				...
				if (targets[i] ~= nil and targets[i].blType == BL_MOB) then
					...
					if r <= 50 then
						if not targets[i]:hasDuration("shock") then
							if checkResist(player, targets[i], "shock") == 1 then
								return
							end
