...
infiltrators_cunning = {
	on_learn = function(player)
		player.registry["learned_infiltrators_cunning"] = 1
	end,
	on_forget = function(player)
		player.registry["learned_infiltrators_cunning"] = 0
	end,
	cast = function(player)
		...
		if player:hasDuration("infiltrators_cunning") then
			player:setDuration("infiltrators_cunning", 0)
			return
		end
		...
		for i = 1, 3 do
			if (mobflankTargets[i] ~= nil) then
				...
				if not mobflankTargets[i]:hasDuration("bleed") then
					...
					if not mobflankTargets[i]:hasDuration("bleed") then
						mobflankTargets[i]:setDuration("bleed", 4000)
					end
				end
			end
		end
