divine_light = {
	broadcast = function()
		...
		if
			realMinute() == 15 or (timeRemaining <= 3600 and timeRemaining >= 3540) or
				(timeRemaining <= 1800 and timeRemaining >= 1740) or
				(timeRemaining <= 600 and timeRemaining >= 540)
		 then
					...
					if not player:hasDuration("divine_light") then
						if player.state ~= 1 then
							if not player:hasDuration("divine_light") then
								...
								player:sendMinitext(
									"You are bathed in Divine Light ((" ..
										mult .. "x EXP))          " .. getTimerValues("divine_light_timer") .. " remaining"
								)
	...
	setMultiplier = function()
		if core.gameRegistry["divine_light_multiplier"] == 0 then
			...
			elseif
				core.gameRegistry["divine_light_lapis_daily"] >= 5000 and core.gameRegistry["divine_light_lapis_daily"] <= 9999
			 then
				core.gameRegistry["divine_light_multiplier"] = 2
			elseif
				core.gameRegistry["divine_light_lapis_daily"] >= 10000 and core.gameRegistry["divine_light_lapis_daily"] <= 29999
			 then
