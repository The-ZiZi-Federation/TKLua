summon_elemental = {
	...
	magicAttack = function(mob, target)
		...
		local damage =
			math.floor(
			(owner.maxMagic * manaMult) + (owner.level * levelMult) + (owner.will * willMult) + (owner.grace * graceMult)
		)
		local anim = 0
		...
		if owner ~= nil then
			...
			if r <= 50 then
				if not target:hasDuration(effect) then
					if checkResist(player, target, effect) == 1 then
						return
					end
