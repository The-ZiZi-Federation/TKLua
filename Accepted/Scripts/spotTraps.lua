seeSpotTraps = function(player)
	...
	elseif (class == 2) then
		...
		for i = 1, #npcs do
			if
				((npcs[i].yname == "DartTrapNpc" or npcs[i].yname == "FlashTrapNpc" or npcs[i].yname == "RepeatingDartTrapNpc" or
					...
					distanceSquare(player, npcs[i], 15))
			 then
				table.insert(npctraps, npcs[i])
			end
		end
	...
	for i = 1, #npctraps do
		player:dropItemXY(99, 1, 0, 0, npctraps[i].m, npctraps[i].x, npctraps[i].y)

		local items = player:getObjectsInCellWithTraps(npctraps[i].m, npctraps[i].x, npctraps[i].y, BL_ITEM)
...
getSpotTrapIds = function(player)
	local items = player:getObjectsInCellWithTraps(player.m, player.x, player.y, BL_ITEM)
...
removeSpotTraps =
	...
	function(player)
		...
		local type = player:menuString("Please choose npc spawners (warriors) or traps (rogues)", {"npc spawners", "traps"})
		...
		elseif type == "traps" then
			for i = 1, #npcBlocks do
				if
					(npcBlocks[i].yname == "DartTrapNpc" or npcBlocks[i].yname == "FlashTrapNpc" or
						...
						npcBlocks[i].yname == "SleepTrapNpc")
				 then
					table.insert(npcsToRemove, npcBlocks[i])
				end
			end
		...
		if #npcsToRemove > 0 then
			for i = 1, #npcsToRemove do
				local items = player:getObjectsInCellWithTraps(npcsToRemove[i].m, npcsToRemove[i].x, npcsToRemove[i].y, BL_ITEM)
