paladin_trainer = {
	click = async(
		function(player, npc)
			...
			if player.class == paladin and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then
			...
			if player.baseClass == 1 and player.class < 5 then
				table.insert(opts, "I want to be a Paladin")
			end
			...
			if menu == "I want to be a Paladin" then
				...
				elseif player.level == 110 then
					player:dialogSeq(
						{
							t,
							name .. "Paladins are Holy Guardians of Morna. With both sword and mind we walk the righteous path.",
							name .. "Becoming a Paladin means a lifetime of devotion and servitude to the Gods.",
							name .. "Are you sure you are capable of commiting to a life of fighting evil wherever if shows it's ugly face?"
						},
						1
					)
					...
					if confirm == "Yes" then
						player:dialogSeq(
							{
								t,
								name .. "You are now one of us. With some time and training you can grow within our ranks.",
								name .. "You are a guiding light in a world of Chaos. Be responsible, Respectful, and use proper Reasoning."
							},
							1
						)
