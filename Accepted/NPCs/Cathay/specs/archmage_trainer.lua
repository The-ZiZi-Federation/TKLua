archmage_trainer = {
	click = async(
		function(player, npc)
			...
			if player.class == archmage and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then
			...
			if player.baseClass == 3 and player.class < 5 then
				table.insert(opts, "I want to be an Archmage")
			end
			...
			if menu == "I want to be an Archmage" then
				...
				elseif player.level == 110 then
					player:dialogSeq(
						{
							t,
							name .. "Archmages are the greatest casters of Morna. With just our mind we can clear any path.",
							name .. "Becoming an Archmage means a lifetime of knowing you are the most powerful and true.",
							name .. "Are you sure you are capable of commiting to a life of knowing no other can ever truly defeat you?"
						},
						1
					)
					confirm =
						player:menuString("Are you sure you want to be an Archmage. You can not undo this choice!", {"Yes", "No"})
					if confirm == "Yes" then
						player:dialogSeq(
							{
								t,
								name .. "Welcome to the coven of greatness. There is just one thing you need to know...",
								name .. "You will need to start at the bottom and work your way into the power. No shortcuts!"
							},
							1
						)
