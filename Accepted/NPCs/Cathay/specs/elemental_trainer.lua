elemental_trainer = {
	click = async(
		function(player, npc)
			...
			if player.class == elemental and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then
			...
			if player.baseClass == 3 and player.class < 5 then
				table.insert(opts, "I want to be an Elemental")
			end
			...
			if menu == "I want to be an Elemental" then
				...
				elseif player.level == 110 then
					player:dialogSeq(
						{
							t,
							name .. "Morna provides us with everything we need to live.",
							name .. "Knowing this, it is safe to say Morna has more power than any mortal.",
							name .. "Dedicating to Elemental Magics is a life of "
						},
						1
					)
					confirm =
						player:menuString("Are you sure you want to be an Elemental. You can not undo this choice!", {"Yes", "No"})
					if confirm == "Yes" then
						player:dialogSeq(
							{
								t,
								name .. "Morna will thank you herself one day I am sure.",
								name .. "Protect this world and she will protect you!"
							},
							1
						)
