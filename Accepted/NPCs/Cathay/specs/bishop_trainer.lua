bishop_trainer = {
	click = async(
		function(player, npc)
			...
			if player.class == bishop and player.mark < 1 and (player.baseHealth >= 457500 or player.baseMagic >= 457500) then
			...
			if player.baseClass == 4 and player.class < 5 then
				table.insert(opts, "I want to be a Bishop")
			end
			...
			if menu == "I want to be a Bishop" then
				...
				elseif player.level == 110 then
					player:dialogSeq(
						{
							t,
							name .. "Well isn't that a bit blunt and forward of you to just request to join us.",
							name .. "You are going to need that kind of confidence to walk the line with us.",
							name .. "We are the grandest healers and prophets that any of the gods could have."
						},
						1
					)
						...
						player:dialogSeq(
							{
								t,
								name ..
									"Without people like us the world will never be saved. We can not guarantee the side we fight for is always right.",
								name .. "So it is up to each of us to determine whether the cause we fight for feels right."
							},
							1
						)
