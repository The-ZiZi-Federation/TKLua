	...
	click = async(
		function(player, npc)
			...
			if (player:hasItem("young_ginseng", 1) == true and not player:hasLegend("aided_chu_rua")) then
				...
				player:addLegend("Aided Chu Rua (" .. curT() .. ")", "aided_chu_rua", 5, 128)
		...
		function(player, npc)
			...
			if speech == "humm dee do dum do hee" then
				...
				if player.quest["wind_armor"] == 0 or player.quest["min_song_asked"] == 0 then
					player:dialogSeq({t, "I really have no idea what you're talking about."}, 0)
					return
				end
				...
				if player.quest["chu_rua_song"] == 1 then
					player:dialogSeq({t, "The dance of the waves does end, a story been retold,"}, 1)
				elseif player.quest["chu_rua_song"] == 2 then
					player:dialogSeq({t, "The dance of the waves does end, a story is retold,"}, 1)
				end
