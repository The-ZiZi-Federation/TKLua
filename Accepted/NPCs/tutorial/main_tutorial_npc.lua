		...
		function(player, npc)
			...
			if player.quest["tutorial_quest"] == 1 then
				...
				player:dialogSeq(
					{
						titem,
						"Go to the blacksmith, get a " ..
							Item(neededItem).name .. ", and bring it back. You'll find it listed under <Peasant's Clothes>"
					},
					1
				)
				player:dialogSeq(
			...
			if player.quest["tutorial_quest"] == 7 then
				...
				if (player:hasLegend("aided_chu_rua")) then
					...
					player:dialogSeq({t, "The Dragon King shall fare better because of you."}, 1)
					return
				end
