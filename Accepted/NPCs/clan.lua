	...
	click = async(
		function(player, npc)
			...
			if (#options > 1) then
				choice =
					player:menuString(
					"I can help you make a clan, but it will cost you. To forge the paperwork properly I'll need " ..
						clanCost .. " gold coins.",
					options
				)
			end
	...
	menu = function(player, npc)
		...
		if (clan.check(player) == true) then --clan
			...
			elseif (menu == "Clan Title") then
				player:dialogSeq(
					{npcG, "Changing clan titles has a cost of 10,000 coins.\n\nOnly proceed if you are willing to pay this fee."},
					1
				)
				...
				if (tempPlayer ~= nil) then
					...
					response =
						player:input("What title would you like to give " .. tempPlayer.clanTitle .. " " .. tempPlayer.name .. "?")
			...
			elseif (menu == "Clan Title") then
				player:dialogSeq(
					{npcG, "Changing clan titles has a cost of 50,000 coins.\n\nOnly proceed if you are willing to pay this fee."},
					1
				)
