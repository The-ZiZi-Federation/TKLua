	...
	broadcastEvent = function(player, npc)
		...
		if player.level < 11 then
			player:dialogSeq(
				{
					t,
					"After you have reached level 11, you will be able to broadcast messages to the RoSTK community regarding an event you are hosting."
				},
				0
			)
			return
		...
		if player.registry["hostCommunityEventTimer"] > os.time() then
			player:dialogSeq(
				{
					t,
					"You have broadcasted to RoSTK recently. Please wait one day."
				},
				0
			)
			return
	...
	novices = function(player, npc, choice)
		...
		if choice == "Help!" then
			player:dialogSeq(
				{
					t,
					"If you have problems that prevent you from playing the game, please ask in the RoSTK discord for help resolving your problem",
					"If you are unsure what you need to do, try reading the Guide or Welcome to RoSTK boards.",
			...
			if countryChoice == "Kugnae" then
				...
				if placeChoice == "IronHeart's home (Tutorial)" then
					...
					player:dialogSeq(
						{
							tironheart,
							"There are many things you need to learn in the world of RoSTK. The tutor can teach you some of those things."
						},
						1
					)
					player:warp(36, 7, 6)
