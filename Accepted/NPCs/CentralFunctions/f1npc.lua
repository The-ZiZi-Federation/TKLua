f1npc = {
	click = async(
		function(player, npc)
			...
			elseif choice == "GM Menu" then
				...
				elseif menu == "Wisdom Star Tools" then
					...
					if subChoice == "Extend Wisdom Star" then
						...
						core.gameRegistry["wisdom_star_timer"] = core.gameRegistry["wisdom_star_timer"] + seconds
			...
			elseif choice == "Spells" then
				local menuChoice = player:menuString("Please choose an option.", {"Learnable/Divine Secret"})
			...
			elseif choice == "Toggles" then
				...
				while s ~= "nothing" do
					local menuChoice =
						player:menuString("Choose a list of toggles you would like to change.", {"Chat Toggles", "Misc Toggles"}, {})
					...
					if menuChoice == "Chat Toggles" then
						...
						local choice = player:menuSeq("Please select a toggle to turn it on and off.", chatsS, {})
					...
					elseif menuChoice == "Misc Toggles" then
						...
						local toggleChoice = player:menuSeq("Please select a toggle to turn it on and off.", toggleS, {})
										...
										" | Will: " ..
											player.baseWill ..
												" | Grace: " ..
													player.baseGrace ..
														"\nVita: " ..
															Tools.formatNumber(player.baseHealth) .. " | Mana: " .. Tools.formatNumber(player.baseMagic) .. "\n"
			...
			elseif choice == "My character webpage" then
				local choice = player:menuString("What do you want to do with your RTK user webpage?", {"Change", "Remove", "Help"})
				...
				if choice == "Change" then
					...
					while s == "buffer" do
						...
						local optChoice =
							player:menuSeq("Please select a feature of your profile you would like to enable/disable.", optsS, {})
						...
						player:dialogSeq(
							{
								...
								"Thank you so much for your donation of " ..
									Tools.formatNumber(amount) .. " Kan. It really means a lot to us and is greatly appreciated."
							},
							0
						)
			...
			elseif choice == "Minigame Stats" then
				...
				if choice == "Carnage" then
					...
					local text =
						"<b>Carnage\n\nWon " ..
						string.format("%.2f", percentWin) ..
							"% of total games played\n\nVictories: " ..
								victories .. "\nLosses: " .. losses .. "\nTotal games played: " .. totalGames
					player:dialogSeq({t, text}, 0)
				elseif choice == "Elixir" then
					local totalGames = player.registry["participated_in_elixir_wars"]
			...
			elseif choice == "Go to Tutor Haven" then
				...
				local warpConfirm = player:menuSeq("Warp to Tutor's Haven?", {"Yes", "No"}, {})
			...
			elseif choice == "Tutor Management" then
				local choice = player:menuString("What would you like to do?", {"Add tutor", "Remove tutor"})
				...
				if choice == "Add tutor" then
					...
					local classChoice = player:menuString("What kind of tutor?", classes)

					target.tutor = 1
					target:addLegend(classChoice .. " Tutor (" .. curT() .. ")", "tutor", 3, 128)
				...
				elseif choice == "Remove tutor" then
					...
					player:dialogSeq({t, target.name .. " has been removed as tutor!"}, 0)
				end
			end
		end
	),
	level5popupDialog = function(player)
		...
		local choice = player:menuSeq("Please select a guild that you'd like to visit.", guilds, {})
