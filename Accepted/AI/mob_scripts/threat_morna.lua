threat = {
	calcHighestThreat = function(mob)
		...
		if
			(mob.target > 0 and targetFacing ~= nil and targetFacing.ID ~= mob.target and
				not mob:checkToObject(mob:getBlock(mob.target)) and
				(#e > 0 or #f > 0 or #g > 0 or #h > 0))
		 then -- or #i > 0 or #j > 0 or #k > 0 or #l > 0)) then
			--[[	
		if (#i > 0) then
			for a = 1, #i do
		...
		if
			(mob.target > 0 and mobTargetFacing ~= nil and mobTargetFacing.ID ~= mob.target and
				not mob:checkToObject(mob:getBlock(mob.target)) and
				(#eMob > 0 or #fMob > 0 or #gMob > 0 or #hMob > 0))
		 then -- or #i > 0 or #j > 0 or #k > 0 or #l > 0)) then
			if (#eMob > 0) then
				for a = 1, #eMob do
