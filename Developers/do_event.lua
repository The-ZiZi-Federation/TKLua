...
canPush = function(block, target, push)
	if block.registry["v_open"] > 0 then
		return false
	end
...
canAmbush = function(block, target, ambush)
	...
	if (block.side == 0) then
		...
		if
			(target.y - 1 > 0 and #checkPCNorth + #checkMobNorth + #checkNPCNorth == 0 and
				target:objectCanMove(target.x, target.y - 1, 0) and
				target:objectCanMoveFrom(target.x, target.y, 0) and
				getPass(target.m, target.x, target.y - 1) == 0 and
				not getWarp(target.m, target.x, target.y - 1))
		 then
			if (ambush == nil) then
				block.side = 2
				block:sendSide()
		...
		elseif
			(target.x + 1 <= block.xmax and #checkPCEast + #checkMobEast + #checkNPCEast == 0 and
				target:objectCanMove(target.x + 1, target.y, 1) and
				target:objectCanMoveFrom(target.x, target.y, 1) and
				getPass(target.m, target.x + 1, target.y) == 0 and
				not getWarp(target.m, target.x + 1, target.y))
		 then
			if (ambush == nil) then
				block.side = 3
				block:sendSide()
		...
		elseif
			(target.x - 1 > 0 and #checkPCWest + #checkMobWest + #checkNPCWest == 0 and
				target:objectCanMove(target.x - 1, target.y, 3) and
				target:objectCanMoveFrom(target.x, target.y, 3) and
				getPass(target.m, target.x - 1, target.y) == 0 and
				not getWarp(target.m, target.x - 1, target.y))
		 then
			if (ambush == nil) then
	...
	elseif (block.side == 1) then
		...
		elseif
			(target.y + 1 <= block.ymax and #checkPCSouth + #checkMobSouth + #checkNPCSouth == 0 and
				target:objectCanMove(target.x, target.y + 1, 2) and
				target:objectCanMoveFrom(target.x, target.y, 2) and
				getPass(target.m, target.x, target.y + 1) == 0 and
				not getWarp(target.m, target.x, target.y + 1))
		 then
			if (ambush == nil) then
